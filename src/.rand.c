...
double get_BoxMuller(DATA *dat, SPDAT *spdat)
{
    ...
    if (spdat->normalSize==262144)
    {
        ...
        for (i=0; i<spdat->normalSize; i++)
        {
            ...
            while (s >= 1);
            spdat->normalNumbs[i]   = u*spdat->weps*sqrt_approx(-2.*log(s)/s);
            spdat->normalNumbs[i+1] = v*spdat->weps*sqrt_approx(-2.*log(s)/s);
            i++;
        }
        spdat->normalSize=0;
    }
